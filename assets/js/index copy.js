...
const blueLine =  ['patna junction', 'akashvani', 'gandhi maidan', 'pmch', 'university', 'moin-ul-haq stadium', 'rajendra nagar', 'malahi pakri', 'khemni chak', 'bhoothnath', 'zero mile', 'new isbt'];
...
function camelCase(str){
  if(str.toLowerCase() == 'pmch' || str.toLowerCase() == 'cnlu' || str.toLowerCase() == 'new isbt'){
    return str.toUpperCase();
  }
  else{
    str = str.toLowerCase(); //making first letter capital and then small
    if(str.indexOf(' ') > -1){
        var camel =  str.split(" ").reduce((s, c) => (s.charAt(0).toUpperCase() + s.slice(1)) + ' ' + (c.charAt(0).toUpperCase() + c.slice(1)));
        return camel;
    }
